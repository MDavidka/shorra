...
export function Dashboard({ username, onResetName }: DashboardProps) {
  ...
  // Filter courses or lessons based on search query
  const filteredCourses = courses.filter((c) =>
    c.name.toLowerCase().includes(searchQuery.toLowerCase().trim())
  );

  const filteredLessons = selectedCourse
    ? selectedCourse.lessons.filter((l) =>
  ...
  // If a lesson is selected, show the Lessonmark view
  ...
  // View 2: Inside a specific Language (e.g. Angol) -> shows lessons list in exact same UI
  if (selectedCourse) {
    return (
      <main className="min-h-screen w-full bg-[#fdfdfd] text-zinc-900 flex flex-col justify-between items-center px-4 py-6 selection:bg-zinc-100 font-sans max-w-lg mx-auto">
        <div className="w-full space-y-5">
          ...
          <header className="flex items-center justify-between pt-2">
            ...
            {/* Leaf Counter (No green color, monochrome) */}
            <button
              type="button"
              className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-zinc-200 bg-zinc-50/60 text-zinc-700 hover:bg-zinc-100 text-sm font-semibold shadow-2xs transition-colors"
            >
              <span>5Levél</span>
          ...
          {/* Lessons List in the exact same card UI */}
  ...
  // View 1: Main Home Screen (Only Language Cards, no lessons list on home screen)
  ...
  return (
    <main className="min-h-screen w-full bg-[#fdfdfd] text-zinc-900 flex flex-col justify-between items-center px-4 py-6 selection:bg-zinc-100 font-sans max-w-lg mx-auto">
      {/* Top Section */}
      <div className="w-full space-y-5">
        {/* Header with Title, Quick Admin links & Leaf Badge (Monochrome, no color) */}
        <header className="flex items-center justify-between pt-2">
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900">
              Shorra
          ...
          {/* Leaf Counter - Clean monochrome, no green coloring */}
          <button
            type="button"
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-zinc-200 bg-zinc-50/60 text-zinc-700 hover:bg-zinc-100 text-sm font-semibold shadow-2xs transition-colors"
          >
        ...
        <div className="pt-2 flex flex-wrap gap-3">
          {primaryCourse && (
            <div
              onClick={() => setSelectedCourse(primaryCourse)}
            ...
            >
              ...
              <div className="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center shrink-0 border border-zinc-100 shadow-2xs">
                <svg viewBox="0 0 60 30" className="w-full h-full object-cover">
                  <clipPath id="s-dash-home">
                    <path d="M0,0 v30 h60 v-30 z" />
                  </clipPath>
                  <clipPath id="t-dash-home">
                    <path d="M30,15 h30 v15 z v15 h-30 z h-30 v-15 z v-15 h30 z" />
                  </clipPath>
                  <g clipPath="url(#s-dash-home)">
